//! Combined CSV data source for multiple files
...
/// Combined CSV data source that merges multiple CSV files
pub struct CombinedCsvSource {
    /// Individual CSV sources
...
impl CombinedCsvSource {
    /// Create a new combined CSV source from multiple files
    pub async fn new(paths: Vec<PathBuf>) -> Result<Self, DataError> {
        if paths.is_empty() {
        ...
        for path in paths {
            let source = CsvSource::new(path).await?;
            sources.push(source);
        }
...
#[async_trait]
impl dv_core::data::DataSource for CombinedCsvSource {
